    ...
    open: function(type, message, defaultValue = '') {
        return new Promise((resolve) => {
            ...
            if (type === 'confirm') {
                ...
                this.btnCancel.classList.remove('hidden');
                // 聰明判斷：如果很長或有換行，自動用大框框，並且自動全選方便用戶複製
...
function renderSettings() {
    const pToggle = document.getElementById('auto-pronounce-toggle'); 
    if (pToggle) pToggle.checked = autoPronounce;
    const sToggle = document.getElementById('sequential-toggle'); 
    if (sToggle) sToggle.checked = isSequentialMode;
}

window.togglePronunciation = function() { 
    autoPronounce = !autoPronounce; 
    localStorage.setItem('sv_autoPronounce', autoPronounce); 
    renderSettings(); 
};

window.toggleSequential = function() { 
...
function endQuiz() {
    if (isGuestMode) {
        SilenModal.confirm("測驗結束。\n\n您要將這份分享的單字庫儲存到您的雲端帳戶中嗎？").then((agreed) => {
            if (agreed) {
                SilenModal.prompt("請為這份單字簿命名：", "分享引入的單字簿").then((newName) => {
                    if (newName) {
                        window.books.push({ id: Date.now(), name: newName, tag: "外部分享", words: guestWords });
                        saveData();
                        SilenModal.alert("已成功匯入單字庫中。").then(() => quitPractice());
                    } else {
                        quitPractice();
                    }
                });
        ...
        SilenModal.alert("測驗結束，做得好。").then(() => quitPractice());
    }
}

// =====================================
// 3. 分享與連網功能 (Share & LZ-String)
// =====================================
window.shareCurrentQuiz = function() {
    if (typeof LZString === 'undefined') { SilenModal.alert("系統壓縮模組載入中，請稍候重試。"); return; }
    
    let wordsToShare = practiceQueue;
    if (practiceQueue.length > 50) {
        SilenModal.alert("提醒：為了最優化傳輸，系統將截取前 50 個單字作為測驗分享包。");
    ...
    try {
        ...
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(shareUrl).then(() => {
                SilenModal.alert("測驗連結已成功複製到剪貼簿。");
            }).catch(() => {
                SilenModal.prompt("請手動複製以下連結：", shareUrl);
            });
        } else {
            SilenModal.prompt("請手動複製以下連結：", shareUrl);
        }
    } catch(e) { 
        SilenModal.alert("產生分享連結失敗"); 
    }
};
...
function checkShareUrl() {
    ...
    if (lzCode) {
        try {
            if (typeof LZString === 'undefined') { setTimeout(checkShareUrl, 100); return true; }
...
window.renderBookList = function() {
    ...
    keys.forEach(k => {
        const header = document.createElement('div'); header.className = 'group-title'; header.innerText = k; list.appendChild(header);
        groups[k].forEach(book => {
            ...
            const editBtn = document.createElement('button'); editBtn.className = 'btn-icon'; editBtn.innerHTML = '編輯'; 
...
window.handleFileUpload = function(event) {
    ...
    reader.onload = function(e) { document.getElementById('import-content').value = e.target.result; event.target.value = ''; };
...
window.addBookSimple = function() {
    ...
    if (!name) { SilenModal.alert("請輸入單字簿名稱"); return; }
    
    window.books.push({ id: Date.now(), name: name, tag: tag, words: [] }); saveData(); 
...
window.addBookWithImport = function() {
    ...
    if (!name) { SilenModal.alert("請輸入單字簿名稱"); return; } 
    if (!rawText) { SilenModal.alert("請輸入轉換內容"); return; }
    ...
    if (newWords.length === 0) { SilenModal.alert("格式解析失敗，請採用「英文 - 中文」結構"); return; }
    
    window.books.push({ id: Date.now(), name: name, tag: tag, words: newWords }); saveData();
    ...
    toggleImportArea(); renderBookList(); SilenModal.alert(`成功匯入 ${newWords.length} 個單字。`);
};

window.toggleExportMenu = function() {
    const menu = document.getElementById('export-menu'); menu.classList.toggle('active');
};

window.exportBook = function(type) {
    const book = window.books.find(b => b.id === currentBookId);
    if (!book || book.words.length === 0) { SilenModal.alert("無可用數據匯出。"); toggleExportMenu(); return; }

    const content = book.words.map(w => `${w.en} - ${w.zh.join(' / ')}`).join('\n');

    if (type === 'copy') {
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(content).then(() => { SilenModal.alert("已複製到剪貼簿。"); })
            .catch(() => { SilenModal.prompt("請複製以下內容：", content); });
        } else {
            SilenModal.prompt("請複製以下內容：", content);
        }
    } else if (type === 'download') {
        const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
        const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url;
        a.download = `${book.name || 'Export'}.txt`; document.body.appendChild(a); a.click();
...
window.deleteCurrentBook = function() {
    SilenModal.confirm('確定要永久刪除此單字簿嗎？').then((agreed) => {
        if(agreed) { 
            window.books = window.books.filter(b => b.id !== currentBookId); 
            selectedBookIds.delete(currentBookId); saveData(); openBookSelect(); 
        }
    });
};
...
window.openEditBook = function(id) { 
    ...
    document.getElementById('export-menu').classList.remove('active'); renderWordList(); switchView('edit'); 
};
...
window.saveBookInfo = function() {
    ...
    if(!newName) { SilenModal.alert('單字簿名稱不能為空。'); return; }
    book.name = newName; book.tag = newTag; saveData(); SilenModal.alert('資訊已更新。');
};
...
function renderWordList() {
    ...
    [...book.words].reverse().forEach((word, index) => {
        ...
        div.innerHTML = `<div><div class="word-en">${word.en}</div><div class="word-zh">${word.zh.join(', ')}</div></div>
...
window.addWord = function() {
    ...
    if(!en || !zhStr) { SilenModal.alert("欄位不完整"); return; }
    const zhArray = zhStr.split(/[;；,，\/]/).map(s => s.trim()).filter(s => s);
    window.books.find(b => b.id === currentBookId).words.push({ en: en, zh: zhArray }); saveData();
...
window.deleteWord = function(index) { window.books.find(b => b.id === currentBookId).words.splice(index, 1); saveData(); renderWordList(); };

function getPracticeWords() {
    if (selectedBookIds.size === 0) { SilenModal.alert("請先選取單字簿範圍。"); return []; }
    let queue = []; window.books.forEach(book => { if (selectedBookIds.has(book.id)) queue.push(...book.words); });
    if (queue.length === 0) { SilenModal.alert("範圍內不含單字。"); return []; } 
    return queue;
}

function getSelectedWordsPool() {
    if (isGuestMode) return guestWords;
    let pool = []; window.books.forEach(book => { if (selectedBookIds.has(book.id)) pool.push(...book.words); }); return pool;
}

// =====================================
// 5. 🚀 雙軌精通模式 (Mastery Mode)
// =====================================
let masteryPool = [], currentMasteryTarget = null, masteryModeType = 'comprehensive', delayWaitTurns = 4;

window.setupMasteryMode = function(type) {
    let words = getPracticeWords(); if(words.length === 0) return;
    masteryModeType = type; masteryPool = words.map(w => ({ en: w.en, zh: w.zh, level: 0, delay: 0 })); masteryPool.sort(() => Math.random() - 0.5);
    ...
    switchView('mastery'); updateMasteryProgress(); nextMasteryTurn();
};
...
function nextMasteryTurn() {
    if (updateMasteryProgress()) {
        hideAllMasteryAreas(); document.getElementById('mastery-success-title').style.color = (masteryModeType === 'comprehensive') ? "#9c27b0" : "#009688";
        setDisplayState('mastery-success-area', true); return;
    }
    hideAllMasteryAreas();

    let l0 = masteryPool.filter(w => w.level === 0);
    if (l0.length > 0) { currentMasteryTarget = l0[0]; showMasteryL0(currentMasteryTarget); return; }
    ...
    if (delayReady.length > 0) {
        ...
        if (masteryModeType === 'comprehensive') showMasteryTyping(currentMasteryTarget, true); 
        else showMasteryMCQ(currentMasteryTarget, true); 
        return;
    }
    ...
    if (active.length > 0) {
        ...
        if (currentMasteryTarget.level === 1) showMasteryMCQ(currentMasteryTarget, false);
        else if (currentMasteryTarget.level === 2) {
            if (masteryModeType === 'comprehensive') showMasteryPuzzle(currentMasteryTarget); else showMasteryMatch(currentMasteryTarget);
        } else if (currentMasteryTarget.level === 3) showMasteryTyping(currentMasteryTarget, false);
        return;
    }
    ...
    if (waiting.length > 0) {
        ...
        if (masteryModeType === 'comprehensive') showMasteryTyping(currentMasteryTarget, true); else showMasteryMCQ(currentMasteryTarget, true);
        return;
    }
}

function tickMasteryDelays() { masteryPool.forEach(w => { if (w.level === 3.5 || w.level === 2.5) { w.delay--; if (w.delay <= 0) w.level = (w.level === 3.5) ? 3.9 : 2.9; } }); }
function hideAllMasteryAreas() { ['mastery-l0-area', 'mastery-mcq-area', 'mastery-match-area', 'mastery-puzzle-area', 'mastery-typing-area', 'mastery-feedback-area', 'mastery-success-area'].forEach(id => { setDisplayState(id, false); }); }
...
function showMasteryL0(word) {
    ...
    if ('speechSynthesis' in window && autoPronounce) {
        window.speechSynthesis.cancel(); let uEn = new SpeechSynthesisUtterance(word.en); uEn.lang = 'en-US'; uEn.rate = 0.95;
        let uZh = new SpeechSynthesisUtterance(word.zh[0]); uZh.lang = 'zh-TW'; window.speechSynthesis.speak(uEn); window.speechSynthesis.speak(uZh);
    }
}

window.masteryL0Next = function() { if ('speechSynthesis' in window) window.speechSynthesis.cancel(); currentMasteryTarget.level = 1; nextMasteryTurn(); };
...
function showMasteryMCQ(word, isDelayed) {
    ...
    if (options.length < 4) { let fallback = getSelectedWordsPool().filter(w => w.en !== word.en).sort(() => Math.random() - 0.5); options.push(...fallback.slice(0, 4 - options.length)); }
    ...
    options.forEach(opt => { let btn = document.createElement('button'); btn.className = 'btn-mcq'; btn.innerText = opt.en; btn.onclick = () => checkMasteryAnswer(opt.en === word.en); optArea.appendChild(btn); });
}
...
function showMasteryMatch(word) {
    ...
    if (selectedDistractors.length < 3) { let globalPool = getSelectedWordsPool().filter(w => w.en !== word.en).sort(() => Math.random() - 0.5); selectedDistractors.push(...globalPool.slice(0, 3 - selectedDistractors.length)); }
    let currentMatchPairs = [word, ...selectedDistractors].slice(0, 4); matchPairsLeft = currentMatchPairs.length;
    renderMatchColumns(currentMatchPairs.map(w => ({ text: w.en, ref: w })).sort(() => Math.random() - 0.5), currentMatchPairs.map(w => ({ text: w.zh[0], ref: w })).sort(() => Math.random() - 0.5));
}

function renderMatchColumns(enList, zhList) {
    const enCol = document.getElementById('match-col-en'); const zhCol = document.getElementById('match-col-zh');
    enCol.innerHTML = ''; zhCol.innerHTML = ''; matchEnSelected = null; matchZhSelected = null;
    enList.forEach((item) => { let btn = document.createElement('button'); btn.className = 'match-btn'; btn.innerText = item.text; btn.onclick = () => handleMatchClick('en', item, btn); enCol.appendChild(btn); });
    zhList.forEach((item) => { let btn = document.createElement('button'); btn.className = 'match-btn'; btn.innerText = item.text; btn.onclick = () => handleMatchClick('zh', item, btn); zhCol.appendChild(btn); });
}
...
function handleMatchClick(type, item, btnElement) {
    ...
    if (type === 'en') {
        ...
        matchEnSelected = { item, btn: btnElement }; btnElement.classList.add('selected');
        if ('speechSynthesis' in window && autoPronounce) { window.speechSynthesis.cancel(); let u = new SpeechSynthesisUtterance(item.text); u.lang = 'en-US'; window.speechSynthesis.speak(u); }
    } else {
        if (matchZhSelected) matchZhSelected.btn.classList.remove('selected');
        matchZhSelected = { item, btn: btnElement }; btnElement.classList.add('selected');
    }
    if (matchEnSelected && matchZhSelected) checkMatchPair();
}
...
function checkMatchPair() {
    ...
    if (en.item.ref.en === zh.item.ref.en) { 
        ...
        if (matchPairsLeft === 0) setTimeout(() => { checkMasteryAnswer(!matchMistake); }, 400);
    } else { 
...
function showMasteryPuzzle(word) {
    ...
    puzzleSourceLetters = letters.map((l, i) => ({ id: i, char: l, used: false })); renderMasteryPuzzleBoard();
}

window.showMasteryPuzzleHint = function() {
    const word = currentMasteryTarget.en; if (word.length <= 2) { document.getElementById('mastery-puzzle-hint-display').innerText = word; return; }
...
function renderMasteryPuzzleBoard() {
    const ansArea = document.getElementById('mastery-puzzle-ans'); const poolArea = document.getElementById('mastery-puzzle-pool');
    ...
    puzzleUserAnswer.forEach((letterObj, idx) => { 
        ...
        tile.onclick = () => { puzzleUserAnswer[idx].used = false; puzzleUserAnswer.splice(idx, 1); renderMasteryPuzzleBoard(); }; ansArea.appendChild(tile); 
    });
    ...
    puzzleSourceLetters.forEach(letterObj => { 
        if (!letterObj.used) { 
            const tile = document.createElement('div'); tile.className = 'letter-tile'; tile.innerText = letterObj.char; 
            tile.onclick = () => { letterObj.used = true; puzzleUserAnswer.push(letterObj); renderMasteryPuzzleBoard(); window.checkMasteryPuzzle(false); }; poolArea.appendChild(tile); 
        } 
    });
}
...
window.checkMasteryPuzzle = function(forced = false) {
    ...
    if (puzzleUserAnswer.length === targetString.length || forced) checkMasteryAnswer(currentString === targetString);
};
...
function showMasteryTyping(word, isDelayed) {
    ...
    input.onkeypress = (e) => { if(e.key === 'Enter') { e.preventDefault(); checkMasteryTyping(); } };
}

window.checkMasteryTyping = function() {
    const val = document.getElementById('mastery-typing-input').value.trim().toLowerCase(); const target = currentMasteryTarget.en.toLowerCase(); checkMasteryAnswer(val === target);
};

function checkMasteryAnswer(isCorrect) {
    hideAllMasteryAreas(); setDisplayState('mastery-feedback-area', true, 'flex');
    ...
    if ('speechSynthesis' in window) { window.speechSynthesis.cancel(); let u = new SpeechSynthesisUtterance(currentMasteryTarget.en); u.lang = 'en-US'; window.speechSynthesis.speak(u); }
    
    tickMasteryDelays(); let lvl = currentMasteryTarget.level;
    ...
    if (masteryModeType === 'comprehensive') {
        if (isCorrect) {
            ...
            else if (lvl === 3.9) { currentMasteryTarget.level = 5; msg.innerText = "通過延遲評測，該單字已完全精通。"; }
        } else {
        ...
        if (isCorrect) {
            ...
            else if (lvl === 2.9) { currentMasteryTarget.level = 4; msg.innerText = "通過延遲評測，單字連接力建立完成。"; }
        } else {
            icon.innerText = '✘'; icon.className = 'big-icon icon-wrong'; status.innerText = '錯誤'; status.className = 'result-status status-wrong';
            currentMasteryTarget.level = 1; msg.innerText = "降級重回 Level 1 視覺辨識。";
        }
...
window.masteryFeedbackNext = function() { nextMasteryTurn(); };
window.replayMasteryAudio = function() { if (currentMasteryTarget && 'speechSynthesis' in window) { window.speechSynthesis.cancel(); let u = new SpeechSynthesisUtterance(currentMasteryTarget.en); u.lang = 'en-US'; window.speechSynthesis.speak(u); } };

// =====================================
// 6. 原版 8 大練習模式 (Original 8 Modes)
// =====================================
window.setupPractice = function(mode) { 
    practiceQueue = getPracticeWords(); if (!practiceQueue.length) return; 
...
function showNextCard() { 
    ...
    document.getElementById('question-display').innerText = q; document.getElementById('feedback-question-copy').innerText = q; 
}

window.showHint = function() { let w = practiceQueue[currentCardIndex].en; document.getElementById('hint-display').innerText = w.length <= 2 ? w : `${w.charAt(0)}${'_'.repeat(w.length-2)}${w.charAt(w.length-1)}`; };

window.checkAnswer = function() { 
    const v = document.getElementById('answer-input').value.trim(); const w = practiceQueue[currentCardIndex]; let c = false; 
    if (v !== '') { if (currentMode === 'zh-to-en') { c = (v.toLowerCase() === w.en.toLowerCase()); } else { c = w.zh.some(m => m.trim().includes(v) && v.length > 0); } } 
    lastAnswerCorrect = c; if (!c && !isSequentialMode) requeueWord(w); showFeedback(c, w); 
};
...
function showFeedback(c, w) { 
    ...
    if (c) { i.innerText = '✔'; i.className = 'big-icon icon-correct'; s.innerText = '正確'; s.className = 'result-status status-correct'; } 
...
window.handleNextClick = function() { 
    ...
    if (isSequentialMode && !lastAnswerCorrect) { 
        SilenModal.alert("評測錯誤，重頭開始。").then(() => { currentCardIndex = 0; completedCount = 0; showNextCard(); });
    } else {
        currentCardIndex++; showNextCard(); 
    }
};

document.getElementById('answer-input').addEventListener('keypress', e => { if (e.key === 'Enter') { e.preventDefault(); window.checkAnswer(); } });

window.setupMultipleChoice = function(mode) { 
    practiceQueue = getPracticeWords(); if (!practiceQueue.length) return; 
    if (new Set(getSelectedWordsPool().map(w => w.en)).size < 4) { SilenModal.alert("單字簿數量不足以生成干擾項選項。"); return; }
    if (!isSequentialMode) practiceQueue.sort(() => Math.random() - 0.5); 
    currentMode = mode; currentCardIndex = 0; initialQueueLength = practiceQueue.length; completedCount = 0; 
    document.getElementById('mcq-mode-display').innerText = mode === 'zh-to-en' ? '中選英' : '英選中'; 
    setDisplayState('mcq-seq-badge', isSequentialMode, 'inline-block'); switchView('mcq'); showMcqNextCard(); 
};

function showMcqNextCard() { 
    if (currentCardIndex >= practiceQueue.length) return endQuiz(); 
    const w = practiceQueue[currentCardIndex]; 
    setDisplayState('mcq-interaction-area', true, 'block'); setDisplayState('mcq-feedback-area', false); 
    ...
    let opts = [w]; let seen = new Set([w.en]); let dis = getSelectedWordsPool().filter(x => !seen.has(x.en)).sort(() => Math.random() - 0.5).slice(0, 3); 
    opts.push(...dis); opts.sort(() => Math.random() - 0.5); const a = document.getElementById('mcq-options-area'); a.innerHTML = ''; 
...
window.checkMcqAnswer = function(c) { 
    lastAnswerCorrect = c; const w = practiceQueue[currentCardIndex]; if (!c && !isSequentialMode) requeueWord(w); 
    setDisplayState('mcq-interaction-area', false); setDisplayState('mcq-feedback-area', true, 'flex'); 
    const i = document.getElementById('mcq-feedback-icon'); const s = document.getElementById('mcq-feedback-status'); document.getElementById('mcq-feedback-answer').innerText = currentMode === 'zh-to-en' ? w.en : w.zh.join(', '); 
    if (c) { i.innerText = '✔'; i.className = 'big-icon icon-correct'; s.innerText = '正確'; s.className = 'result-status status-correct'; } 
    else { i.innerText = '✘'; i.className = 'big-icon icon-wrong'; s.innerText = '錯誤'; s.className = 'result-status status-wrong'; } 
    speakEnglishWord(w.en); 
};
...
window.handleMcqNextClick = function() { 
    if (lastAnswerCorrect) completedCount++; 
    if (isSequentialMode && !lastAnswerCorrect) { SilenModal.alert("評測錯誤，重頭開始。").then(() => { currentCardIndex = 0; completedCount = 0; showMcqNextCard(); }); } 
    else { currentCardIndex++; showMcqNextCard(); }
};

window.setupSpeakingMode = function() { 
    if (!recognition) { SilenModal.alert("當前核心環境不支援 SpeechRecognition 語音介面。"); return; }
    practiceQueue = getPracticeWords(); if (!practiceQueue.length) return; 
    if (!isSequentialMode) practiceQueue.sort(() => Math.random() - 0.5); 
...
window.startSpeechRecognition = function() { 
    const b = document.getElementById('mic-btn'); const s = document.getElementById('speaking-status'); 
    ...
    recognition.onresult = (e) => { 
        ...
        if (h === t || h.includes(t) || t.includes(h)) { 
            lastAnswerCorrect = true; let fs = Math.round(c * 100); if (fs < 50) fs = 80; sd.innerText = `${fs} 分`; sd.style.color = 'var(--success)'; md.innerText = '發音標準'; hd.innerText = `捕獲音訊: "${h}"`; 
        } else { 
            lastAnswerCorrect = false; sd.innerText = `0 分`; sd.style.color = 'var(--error)'; md.innerText = '識別不匹配'; hd.innerText = `捕獲音訊: "${h}"`; 
            if (!isSequentialMode) requeueWord(practiceQueue[currentCardIndex]); 
        } 
    }; 
    recognition.onerror = () => { b.classList.remove('listening'); s.innerText = '音訊解碼失敗，請重新觸發。'; }; 
    recognition.onspeechend = () => { recognition.stop(); b.classList.remove('listening'); }; 
};

window.handleSpeakingNextClick = function() { 
    if (lastAnswerCorrect) completedCount++; 
    if (isSequentialMode && !lastAnswerCorrect) { SilenModal.alert('重頭開始。').then(() => { currentCardIndex = 0; completedCount = 0; showNextSpeakingCard(); }); } 
...
window.setupPuzzleMode = function() { 
    practiceQueue = getPracticeWords(); if (!practiceQueue.length) return; 
    if (!isSequentialMode) practiceQueue.sort(() => Math.random() - 0.5); 
    currentCardIndex = 0; setDisplayState('puzzle-seq-badge', isSequentialMode, 'inline-block'); switchView('puzzle'); loadPuzzleLevel(); 
};

function loadPuzzleLevel() { 
    if (currentCardIndex >= practiceQueue.length) return endQuiz(); 
    puzzleCurrentWord = practiceQueue[currentCardIndex]; puzzleUserAnswer = []; let ls = puzzleCurrentWord.en.toLowerCase().split(''); 
    for (let i = ls.length - 1; i > 0; i--) { let j = Math.floor(Math.random() * (i + 1)); [ls[i], ls[j]] = [ls[j], ls[i]]; } 
    ...
    document.getElementById('puzzle-progress').innerText = isSequentialMode ? `第 ${currentCardIndex+1} 關` : `${currentCardIndex+1}/${practiceQueue.length}`; renderPuzzleBoard(); 
}

window.showPuzzleHint = function() { let w = puzzleCurrentWord.en; document.getElementById('puzzle-hint-display').innerText = w.length <= 2 ? w : `${w.charAt(0)}${'_'.repeat(w.length-2)}${w.charAt(w.length-1)}`; };
...
function renderPuzzleBoard() { 
    ...
    puzzleUserAnswer.forEach((o, i) => { let t = document.createElement('div'); t.className = 'letter-tile'; t.innerText = o.char; t.onclick = () => { puzzleUserAnswer[i].used = false; puzzleUserAnswer.splice(i, 1); renderPuzzleBoard(); }; a.appendChild(t); }); 
    ...
    puzzleSourceLetters.forEach(o => { if (!o.used) { let t = document.createElement('div'); t.className = 'letter-tile'; t.innerText = o.char; t.onclick = () => { o.used = true; puzzleUserAnswer.push(o); renderPuzzleBoard(); window.checkPuzzleState(false); }; p.appendChild(t); } }); 
}

window.checkPuzzleState = function(f = false) { 
    let cs = puzzleUserAnswer.map(o => o.char).join(''); let ts = puzzleCurrentWord.en.toLowerCase(); let m = document.getElementById('puzzle-message'); 
    if (cs.length === ts.length || f) { 
        if (cs === ts) { 
            m.className = 'result-msg result-correct'; m.innerText = '正確'; speakEnglishWord(ts); 
            setTimeout(() => { currentCardIndex++; loadPuzzleLevel(); }, 800); 
        } else { 
            if (isSequentialMode) { 
                m.className = 'result-msg result-wrong'; m.innerText = `錯誤，答案為 ${ts}。`; speakEnglishWord(ts); 
                setTimeout(() => { currentCardIndex = 0; loadPuzzleLevel(); }, 2000); 
            } else { 
                if (f) { 
                    m.className = 'result-msg result-wrong'; m.innerText = `錯誤，答案為 ${ts}`; speakEnglishWord(ts); requeueWord(puzzleCurrentWord); 
...
window.setupMemoryMode = function() { 
    let p = getPracticeWords(); if (p.length < 2) { SilenModal.alert("生成記憶矩陣單字數量不足。"); return; } 
    ...
    switchView('memory'); renderMemoryBoard(); document.getElementById('memory-message').innerText = '請選取卡片'; 
};

function setupMemoryModeGuest() { 
    let p = [...practiceQueue]; if (p.length < 2) { SilenModal.alert("生成記憶矩陣單字數量不足。"); return; } 
    p.sort(() => Math.random() - 0.5); let sw = p.slice(0, 8); memoryCards = []; 
    sw.forEach(w => { memoryCards.push({ id: w.en, content: w.en, type: 'en', matched: false }); memoryCards.push({ id: w.en, content: w.zh[0], type: 'zh', matched: false }); }); 
    memoryCards.sort(() => Math.random() - 0.5); memoryFlipped = []; memoryLocked = false; memoryMatchedCount = 0; 
    switchView('memory'); renderMemoryBoard(); document.getElementById('memory-message').innerText = '請選取卡片'; 
...
function renderMemoryBoard() { 
    ...
    memoryCards.forEach((c, i) => { let d = document.createElement('div'); d.className = `memory-card ${c.matched ? 'matched' : ''}`; d.innerHTML = `<div class="memory-inner"><div class="memory-front">${c.content}</div><div class="memory-back">?</div></div>`; d.onclick = () => flipCard(i); b.appendChild(d); }); 
}

function flipCard(i) { 
    if (memoryLocked || memoryCards[i].matched || memoryFlipped.includes(i)) return; 
    document.getElementById('memory-board').children[i].classList.add('flipped'); memoryFlipped.push(i); if (memoryFlipped.length === 2) checkMemoryMatch(); 
}

function checkMemoryMatch() { 
    memoryLocked = true; let [i1, i2] = memoryFlipped; let c1 = memoryCards[i1]; let c2 = memoryCards[i2]; let m = document.getElementById('memory-message'); 
    if (c1.id === c2.id) { 
        ...
        m.innerText = '矩陣配對成功'; m.className = 'result-msg result-correct'; speakEnglishWord(c1.id); memoryFlipped = []; memoryLocked = false; 
...
window.setupYouglishMode = function() { 
    practiceQueue = getPracticeWords(); if (!practiceQueue.length) return; 
    practiceQueue.sort(() => Math.random() - 0.5); currentCardIndex = 0; switchView('youglish'); loadYouglishCard(); 
};

function loadYouglishCard() { 
    if (!practiceQueue[currentCardIndex]) return; const w = practiceQueue[currentCardIndex]; 
    document.getElementById('yg-word').innerText = w.en; document.getElementById('yg-zh').innerText = w.zh.join(' / '); document.getElementById('yg-progress').innerText = `${currentCardIndex+1}/${practiceQueue.length}`; 
    document.getElementById('yg-link-word').innerText = w.en; document.getElementById('yg-link').href = `https://youglish.com/pronounce/${encodeURIComponent(w.en)}/english`; 
}

window.nextYouglishCard = function() { if (currentCardIndex < practiceQueue.length - 1) { currentCardIndex++; loadYouglishCard(); } else { endQuiz(); } };
window.prevYouglishCard = function() { if (currentCardIndex > 0) { currentCardIndex--; loadYouglishCard(); } else { SilenModal.alert("已達佇列首端。"); } };

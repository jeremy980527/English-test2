...
function saveData() { 
    localStorage.setItem('sv_books', JSON.stringify(books)); 
}

const views = ['home', 'book-select', 'edit', 'practice', 'mcq', 'speaking', 'puzzle', 'memory', 'youglish', 'mastery'];
...
function updateHomeSummary() {
    ...
    if (selectedCount === 0) {
        summaryEl.innerHTML = '<span style="color:var(--text-sub);">尚未選擇單字簿，請先進入管理區選擇喔！</span>';
    } else {
        summaryEl.innerHTML = `已準備好 <span style="color:var(--accent); font-weight:bold; font-size:1.3rem;">${selectedCount}</span> 本單字簿，共 <span style="color:var(--accent); font-weight:bold; font-size:1.3rem;">${wordCount}</span> 字`;
    }
}
...
function addBookSimple() {
    ...
    books.push({ id: Date.now(), name: name, tag: tag, words: [] }); 
    saveData(); 
...
function addBookWithImport() {
    ...
    if (!name) return alert("請輸入單字簿名稱"); 
    if (!rawText) return alert("請貼上單字列表");
    ...
    if (newWords.length === 0) {
        return alert("無法識別任何單字，請檢查格式是否為「英文 - 中文」");
    }
    
    books.push({ id: Date.now(), name: name, tag: tag, words: newWords }); 
    saveData();
    ...
    toggleImportArea(); 
    renderBookList(); 
    alert(`成功建立單字簿「${name}」，並匯入 ${newWords.length} 個單字！`);
}
...
function exportBook(type) {
    ...
    if (type === 'copy') {
        ...
        a.href = url;
        a.download = `${book.name || 'SilenVocab_Export'}.txt`;
...
function renderWordList() {
    ...
    [...book.words].reverse().forEach((word, index) => {
        ...
        div.innerHTML = `
            ...
            <button class="btn-icon btn-delete" onclick="deleteWord(${book.words.length - 1 - index})">✕</button>
        `;
        list.appendChild(div);
    });
}
...
function addWord() {
    ...
    books.find(b => b.id === currentBookId).words.push({ en: en, zh: zhArray }); 
    saveData();
...
function deleteWord(index) { 
    books.find(b => b.id === currentBookId).words.splice(index, 1); 
    saveData(); 
    renderWordList(); 
}
...
function getPracticeWords() {
    if (selectedBookIds.size === 0) { 
        alert("請先進入「選擇與管理單字簿」勾選你要練習的範圍喔！"); 
        return []; 
    }
    let queue = []; 
    books.forEach(book => { 
...
function setupMasteryMode(type) {
...
function updateMasteryProgress() {
    ...
    let total = masteryPool.length;
    let pct = (mastered / total) * 100;
    
    document.getElementById('mastery-progress-bar').style.width = pct + '%';
    document.getElementById('mastery-status-text').innerText = `進度: ${mastered} / ${total}`;
...
function showMasteryMCQ(word, isDelayed) {
    ...
    let badge = document.getElementById('mastery-mcq-badge');
    if (isDelayed) {
        badge.innerText = "Lv 3: 延遲固化 (畢業測驗)";
    } else {
        badge.innerText = "Lv 1: 視覺辨識";
    }
    
    document.getElementById('mastery-mcq-q').innerText = word.zh.join(' / ');
    
    let options = [word];
    let distractors = masteryPool.filter(w => w.en !== word.en);
    distractors.sort(() => Math.random() - 0.5);
    options.push(...distractors.slice(0, 3));
    
    if (options.length < 4) {
        let fallback = getSelectedWordsPool().filter(w => w.en !== word.en);
...
let matchEnSelected = null;
let matchZhSelected = null;
let matchMistake = false;
let matchPairsLeft = 4;

function showMasteryMatch(word) {
    setDisplayState('mastery-match-area', true);
    matchMistake = false;
    
    let pool = masteryPool.filter(w => w.en !== word.en);
    pool.sort(() => Math.random() - 0.5);
    let selectedDistractors = pool.slice(0, 3);
    
    if (selectedDistractors.length < 3) {
        let globalPool = getSelectedWordsPool().filter(w => w.en !== word.en);
    ...
    let enList = currentMatchPairs.map(w => ({ text: w.en, ref: w })).sort(() => Math.random() - 0.5);
    let zhList = currentMatchPairs.map(w => ({ text: w.zh[0], ref: w })).sort(() => Math.random() - 0.5);
    
    renderMatchColumns(enList, zhList);
}
...
function checkMatchPair() {
    let en = matchEnSelected; 
    let zh = matchZhSelected;
...
function showMasteryPuzzleHint() {
...
function checkMasteryPuzzle(forced = false) {
...
function showMasteryTyping(word, isDelayed) {
    ...
    document.getElementById('mastery-typing-badge').innerText = isDelayed ? "Lv 5: 延遲固化 (畢業測驗)" : "Lv 3: 主動輸出";
...
function masteryFeedbackNext() {
    nextMasteryTurn();
}

function replayMasteryAudio() {
...
function showMcqNextCard() { 
    ...
    opts.forEach(o => { 
        ...
        b.onclick = () => checkMcqAnswer(o.en === w.en); 
        a.appendChild(b); 
    }); 
}

function checkMcqAnswer(c) { 
    lastAnswerCorrect = c; 
    const w = practiceQueue[currentCardIndex]; 
    if (!c && !isSequentialMode) requeueWord(w); 
...
function showNextSpeakingCard() { 
    ...
    document.getElementById('speaking-status').innerText = '點擊麥克風'; 
...
function checkPuzzleState(f = false) { 
...
function setupMemoryMode() { 
    ...
    if (p.length < 2) return alert("單字過少"); 
    ...
    switchView('memory'); 
    renderMemoryBoard(); 
    document.getElementById('memory-message').innerText = '點擊翻卡'; 
}
